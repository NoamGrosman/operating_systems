...
// Convert MTBlock to payload pointer
static void* mt_block_to_payload(MTBlock* b) {
    return (void*)(b + 1);
}

// Find best fit block in a region
...
// Find block by payload in a region
...
// Initialize a region with given memory
...
static MemRegion* mt_create_extra_region(void) {
    ...
    // Allocate memory for the regions heap space
...
void* customMTMalloc(size_t size) {
    ...
    // Check this wont exceed region size
